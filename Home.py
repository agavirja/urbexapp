...
# Get-ChildItem -Path D:\Dropbox\Empresa\Urbex\_APP_heroku -Recurse -Filter *.py | ForEach-Object { if (Get-Content $_.FullName | Select-String -Pattern 'urbex.com.co' -Quiet) { $_.FullName } }

# Reemplazar "urbex.com.co" por "urbex.com.co" o al reves en los archivos donde esta la palabra
# Get-ChildItem -Path D:\Dropbox\Empresa\Urbex\_APP_heroku -Recurse -Filter *.py | ForEach-Object {(Get-Content $_.FullName) | ForEach-Object {$_ -replace 'urbex.com.co', 'urbex.com.co'} | Set-Content $_.FullName}
