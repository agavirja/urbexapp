    ...
    if not data.empty:
        ...
        for idd,items in data.iterrows():
            ...
            <html>
                <body>
                    <div id="popupContent" style="cursor:pointer; display: flex; flex-direction: column; flex: 1;width:200px;">
                        <a href="http://urbex.com.co/Busqueda_avanzada?type=predio&code={items['barmanpre']}&vartype=barmanpre&token={st.session_state.token}" target="_blank" style="color: black;">
                            {titular}
                            {direccion}
                            {barrio}
                            {fecha}
