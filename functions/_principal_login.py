...
def main():
    ...
    if controller.ready():
        ...
        if isinstance(session_token,str) and session_token!='':
            st.session_state.token, st.session_state.access = datos_usuario(session_token)
        
    if not st.session_state.access and controller.ready(): 
        login_signup(controller)
        
def login_signup(controller):
    
    #-------------------------------------------------------------------------#
    # Estilos     
    colorlogo  = "negativo"
    if st.session_state.access is False and controller.ready():
        background = """
        .stApp {
    ...
    #-------------------------------------------------------------------------#
    # Login
    if st.session_state.login and st.session_state.access is False and controller.ready():
                    ...
                    if submit:
                        with st.spinner('Verificando'):
                            ...
                            if r['status']==200 and st.session_state.access:
                                ...
                                controller["session_token"] = email
                                controller.save() 
                                st.rerun()
    ...
    #-------------------------------------------------------------------------#
    # Signup
    if st.session_state.signin and st.session_state.access is False  and controller.ready(): 
        col1, col2,col3 = st.columns([0.5,0.4,0.1])
        with col2:
            with st.container():
    ...
    #-------------------------------------------------------------------------#
    # Olvido clave                        
    if st.session_state.forgot  and st.session_state.access is False and controller.ready():
        col1, col2,col3 = st.columns([0.5,0.4,0.1])
        with col2:
            email = st.text_input("Email")
            ...
            if st.session_state.forgot_codigo:
                ...
                if st.button("Confirmar codigo"):
                    ...
                    if r['status']==200:
                        ...
                        st.session_state.signin = False
                        st.session_state.forgot = False
                        st.session_state.forgot_codigo = False
                        st.rerun()
                    else: 
                        st.error(r['message']) 
